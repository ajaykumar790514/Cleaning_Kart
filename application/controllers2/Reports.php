...
class Reports extends CI_Controller
{
    ...
    public function product_stock_report($action = null, $p1 = null, $p2 = null, $p3 = null, $p4 = null)
    {
        switch ($action) {
            case null:
            ...
            case 'tb':
                ...
                $shop_id     = $_SESSION['user_data']['id'];
                $config["base_url"]         = base_url() . "product-stock-report/tb/" . $parent_id . "/" . $cat_id . "/" . $search;
                $config["total_rows"]       = $this->reports_model->get_product_stock_report($parent_id, $cat_id, $shop_id, $search);
                $data['total_rows']         = $config["total_rows"];
                $config["per_page"]         = 10;
                $config["uri_segment"]      = 6;
                $config['attributes']       = array('class' => 'pag-link');
                ...
                $data['per_page']           = $config["per_page"];
                $data['parent_cat'] = $this->master_model->get_data('products_category', 'is_parent', '0');
                $data['stock_report']           = $this->reports_model->get_product_stock_report($parent_id, $cat_id, $shop_id, $search, $config["per_page"], $page);
                $data['stock_result']           = $this->reports_model->get_product_stock_report_result($cat_id, $shop_id, $search);
    ...
    public function purchase_report($action = null, $p1 = null, $p2 = null, $p3 = null, $p4 = null, $p5 = null, $p6 = null, $p7 = null, $p8 = null, $p9 = null)
    {
        ...
        switch ($action) {
            ...
            case 'export_to_excel':
                ...
                $sheet->setCellValue('D1', 'Software Parent Category');
                $sheet->setCellValue('E1', 'Software Sub-Category');
                $sheet->setCellValue('F1', 'Product Category');
                $sheet->setCellValue('G1', 'Product Code');
                $sheet->setCellValue('I1', 'Product Name');
                $sheet->setCellValue('J1', 'Expiry Date');
                $sheet->setCellValue('K1', 'Hsn/Sac');
                $sheet->setCellValue('L1', 'Vendor Name');
                $sheet->setCellValue('M1', 'Invoice no');
                $sheet->setCellValue('N1', 'MRP');
                $sheet->setCellValue('O1', 'Total MRP Value');
                $sheet->setCellValue('P1', 'Quantity');
                $sheet->setCellValue('Q1', 'Unit Type');
                $sheet->setCellValue('R1', 'Unit Price without tax');
                $sheet->setCellValue('S1', 'Total without tax');
                $sheet->setCellValue('U1', 'UP/EXUP');
                $sheet->setCellValue('V1', 'Tax rate');
                $sheet->setCellValue('W1', 'Igst rate');
                $sheet->setCellValue('X1', 'Cgst rate');
                $sheet->setCellValue('Y1', 'Sgst rate');
                $sheet->setCellValue('Z1', 'Igst value');
                $sheet->setCellValue('AA1', 'Cgst value');
                $sheet->setCellValue('AB1', 'Sgst value');
                $sheet->setCellValue('AC1', 'Total tax');
                $sheet->setCellValue('AD1', 'Total value with tax');
                $sheet->setCellValue('AE1', 'Product Price / Piece');
                $sheet->setCellValue('AF1', 'Additional Discount');
                $sheet->setCellValue('AG1', 'Vendor Code');
                $sheet->setCellValue('AH1', 'GSTIN');
                $sheet->setCellValue('AI1', 'Address');
                $sheet->setCellValue('AJ1', 'Purchase Type');
                $count = 2;
                $i = 1;
                foreach ($result as $value) {
                    ...
                    $sheet->setCellValue('C' . $count, $value->brand_name);
                    $sheet->setCellValue('D' . $count, $Parent_cat_name);
                    $sheet->setCellValue('E' . $count, $subcat_name);
                    $sheet->setCellValue('F' . $count, $cat_name);
                    $sheet->setCellValue('G' . $count, $value->product_code);
                    $sheet->setCellValue('H' . $count, $value->product_name);
                    $sheet->setCellValue('I' . $count, $value->expiry_date);
                    $sheet->setCellValue('J' . $count, $value->sku);
                    $sheet->setCellValue('K' . $count, $value->vendor_name);
                    $sheet->setCellValue('L' . $count, $value->invoice_no);
                    $sheet->setCellValue('M' . $count, $value->mrp);
                    $sheet->setCellValue('N' . $count, ($value->mrp*$value->qty));
                    $sheet->setCellValue('O' . $count, $value->qty);
                    $sheet->setCellValue('P' . $count, $value->unit_type);
                    $sheet->setCellValue('Q' . $count, round($unit_price_without_tax, 2));
                    $sheet->setCellValue('R' . $count, round($total_without_tax, 2));
                    $sheet->setCellValue('T' . $count, $up_exup);
                    $sheet->setCellValue('U' . $count, round($value->purchase_rate, 2));
                    $sheet->setCellValue('V' . $count, round($value->tax_value, 2));
                    $sheet->setCellValue('W' . $count, round($igst, 2));
                    $sheet->setCellValue('X' . $count, round($cgst, 2));
                    $sheet->setCellValue('Y' . $count, round($sgst, 2));
                    $sheet->setCellValue('Z' . $count, round($igst_val, 2));
                    $sheet->setCellValue('AA' . $count, round($cgst_val, 2));
                    $sheet->setCellValue('AB' . $count, round($sgst_val, 2));
                    $sheet->setCellValue('AC' . $count, round($total_tax, 2));
                    $sheet->setCellValue('AD' . $count, round($total_value_with_tax, 2));
                    $sheet->setCellValue('AE' . $count, round($value->purchase_rate, 2));
                    $sheet->setCellValue('AF' . $count, $value->AdditionalDiscount);
                    $sheet->setCellValue('AG' . $count, $value->vendor_code);
                    $sheet->setCellValue('AH' . $count, $value->gstin);
                    $sheet->setCellValue('AI' . $count, $value->vendor_address);
                    $sheet->setCellValue('AJ' . $count, 'Bill');
                    $count++;
                }
                $writer = new Xlsx($spreadsheet);
