...
class Stocks extends CI_Controller
{
    ...
    {
        $pro = $this->shops_inventory_model->getProductimg($_GET['product_id']);
        if (@$pro->img) {
            echo "<img src='$pro->img' style='max-height:155px'>";
        }
        else{
            echo '';
        }
    ...
    public function insertStockData()
    {
        ...
        $ledger['customer_id']  = $_POST['item']['vendor_id'];
        $ledger['order_id']     = NULL;
        $ledger['inventory_id'] = $insert_stock;
        $ledger['dr']           = $_POST['item']['total_value'];
        $ledger['reference_no'] = $_POST['item']['invoice_no'];
        $ledger['PaymentDate']  = $_POST['item']['invoice_date'];
        $ledger['shop_id']      = $_POST['shop_id'];
        $ledger['txn_type']     = 4;
    ...
    {
         if ($this->db->get_where('pos_order_items',['inventory_id'=> $_POST['item']['id'] ])) {
            echo json_encode(['res'=>'error','msg'=>"stock can't update after sales"]);
            die();
        }
        ...
        if ($checkExistingData !== FALSE) {
            ...
            $ledger_cond['inventory_id']    = $_POST['item']['id'];
    ...
    public function deleteStockData()
    {
        ...
        if ($this->shops_inventory_model->delete_data('shops_inventory', $shop_inventory_id)) {
            ...
            // $ledger_cond['type']         = 'Purch';
            // $ledger_cond['inventory_id'] = $_POST['item']['id'];
        
            // $this->load->model('cash_register_model');
            // $this->cash_register_model->deleteRow($ledger_cond);
